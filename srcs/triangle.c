...
/*   Updated: 2021/05/10 15:38:58 by tvanelst         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_abc(double *factors, t_vec u, t_vec v, t_vec w)
{
	...
	factors[1] = get_factor((double []){b11, m22, b21, m12},
			m11 * m22 - m12 * m12);
	factors[2] = get_factor((double []){m11, b21, m12, b11},
			m11 * m22 - m12 * m12);
	factors[0] = 1 - factors[1] - factors[2];
}
...
int	tr_intersection(t_ray ray, t_triangle tr, t_intersection *x)
{
	...
	i = -1;
	while (++i < 3)
		if (factors[i] < 0 || 1 < factors[i] || my_t >= x->d)
			return (0);
	x->d = my_t;
	x->p = p;
	x->n = n;
